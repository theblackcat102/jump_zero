...
from models.dualresnetv2 import DualResNet, alpha_loss, set_learning_rate
    ...
    for epoch in range(epochs):
        with tqdm(total=batch_num, ncols=150) as t:
            ...
            for batch in dataloader:
                ...
                optimizer.zero_grad()
                set_learning_rate(optimizer, max(min(lr*lr_multiplier, 0.01), 0.0005) )
                ...
                writer.add_scalar('learning_rate',  max(min(lr*lr_multiplier, 0.01), 0.0005), num_iter)
...
if __name__ == "__main__":
    ...
    name = 'DualResNetv3_1.pt'
    writer_idx = 0
    l2_const = L2_REG
    epochs = 1 # number of epoch training
    batch_size = 512
    num_iter = writer_idx
    n_playout = PLAYOUT_ROUND+10
    ...
    while True:
        ...
        if skip_first:
            ...
            model.eval()
            # pool_selfplay(model, cpu, rounds=8, n_playout=n_playout)
            single_self_play(1, model, n_playout=n_playout)
